...
function Category({ id, count, name, color, className, active, onClick, onDelClick, onChangeClick }) {
    ...
    const onChange = (e) => {
        e.stopPropagation();
        if (isChange) {
            onChangeClick(id, newName, newColor);
            setIsChange(false);
        } else {
            setNewName(name);
            setNewColor(color);
